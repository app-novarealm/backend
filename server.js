...
async function getVerificationCode(email, verificationCode) {
  try {
    const mailOptions = {
      from: `"Nova Realm Verification" ${process.env.EMAIL_USER}`,
      to: email,
      subject: 'Your Verification Code for Nova Realm',
      text: `Hello,\n\nYour verification code is: ${verificationCode}\nThis code is valid for 10 minutes.\n\nIf you did not request this, you can safely ignore this email.\n\nThanks,\nNova Realm Team`,
      html: `
        <div style="font-family: Arial, sans-serif; color: #333; font-size: 16px; line-height: 1.6;">
          <p>Hello,</p>
          <p>Here is your verification code for <strong>Nova Realm</strong>:</p>
          ...
          <p style="margin-top: 20px;">Thanks,<br/><strong>Nova Realm Team</strong></p>
          <p style="font-size: 12px; color: #999; margin-top: 40px;">
            This email was sent by Nova Realm. If you have any questions, contact us at contact.novarealm@gmail.com
          </p>
        </div>
      `,
    };
...
const WALLET_ADDRESS = "ABV6arHyPaFdcrXCAWT6L6BAHHwd4L1Fp6RFhr6EjXB1";

app.post("/verify-transaction", verifyAccessToken, async (req, res) => {
  let { signature, reference, amount, fromPubkey, taskId, reward } = req.body;
  const userId = req.user?.id;

  try {
    // 1) If no signature provided, but a reference exists, look up the signature
    if (!signature && reference) {
      const refKey = new PublicKey(reference);
      const sigInfos = await connection.getSignaturesForAddress(refKey, { limit: 1 });
      if (sigInfos.length === 0) {
        return res.status(400).json({ success: false, message: "❌ No transaction found for this reference" });
      }
      signature = sigInfos[0].signature;
    }

    if (!signature) {
      return res.status(400).json({ success: false, message: "❌ Missing signature or reference" });
    }

    // 2) Fetch and parse the transaction
    ...
    // 3) Find the SOL-transfer instruction
    ...
    if (sol < amount) {
      return res.status(400).json({ success: false, message: "❌ Insufficient SOL sent" });
    }

    // 5) All good → record it
    await pool.query(
      "INSERT INTO completed_investment (user_id, task_id, address, reward_point) VALUES ($1,$2,$3,$4)",
      [userId, taskId, fromPubkey, reward]
    );
